...
- **UI screenshots must stay current.** `docs/screenshots/` holds version-stamped phone screenshots (`<Screen>-<client version>.png`) embedded in README. Any change to user-visible UI (copy, icons, layout, new screens) must check the affected screenshots: highlight it to user, user will take them manually. Avoid credentials, private data.
...
- **Follow the "Before committing" workflow in `CONTRIBUTING.md`** before considering any change done.
